...
const EVENTS = [
  { week: 1, type: 'expense', text: "Your textbooks cost $150 more than expected.", amount: 150 },
  { week: 1, type: 'decision', text: "Flatmate wants to split a Pak'nSave grocery run. Your share: $55.", options: [
    { text: "Join the bulk buy ($55) — better value per item", cost: 55, happiness: 8, wisdom: 12 },
    { text: "Buy your own groceries separately", cost: 40, happiness: 3, wisdom: 3 },
  ]},
  { week: 2, type: 'decision', text: "Friend's birthday — group dinner invitation.", options: [
    { text: "Go to the $55 dinner", cost: 55, happiness: 15, wisdom: 0 },
    { text: "Suggest a $20 picnic instead", cost: 20, happiness: 10, wisdom: 10 },
    { text: "Skip it", cost: 0, happiness: -5, wisdom: 5 },
  ]},
  { week: 3, type: 'expense', text: "Power bill arrived — winter heaters pushed it up. Your share: $117.", amount: 117 },
  { week: 3, type: 'decision', text: "Flash sale: jacket 50% off ($45).", options: [
    { text: "Buy it — great deal!", cost: 45, happiness: 10, wisdom: -5 },
    { text: "Wishlist it — check back next month", cost: 0, happiness: 2, wisdom: 15 },
  ]},
  { week: 4, type: 'income', text: "You picked up an extra shift at work!", amount: 100 },
  { week: 4, type: 'decision', text: "Flat wants group takeaway to celebrate semester end.", options: [
    { text: "Join the $32 takeaway", cost: 32, happiness: 12, wisdom: 0 },
    { text: "Suggest cooking together ($10 each)", cost: 10, happiness: 10, wisdom: 12 },
  ]},
];

const INITIAL_BUDGET = 1920; // 4 weeks × $480
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm space-y-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-700 mt-1">4 weeks · $480/week income · 7 events</p>
          </div>
        </div>
        ...
        <div className="bg-[#EEF5F0] rounded-xl p-3 text-sm text-gray-600 italic">
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
        <div className="bg-[#1B4332] rounded-2xl p-5 text-center">
          ...
          <p className="text-white/80 text-sm font-semibold">Semester complete!</p>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-white rounded-2xl p-3 text-center shadow-sm"><DollarSign className="w-5 h-5 text-[#1B4332] mx-auto mb-1"/><p className="font-bold text-[#1A1A1A]">${remaining.toFixed(0)}</p><p className="text-sm text-gray-600">Left over</p></div>
          <div className="bg-white rounded-2xl p-3 text-center shadow-sm"><Heart className="w-5 h-5 text-red-400 mx-auto mb-1"/><p className="font-bold text-[#1A1A1A]">{happiness}%</p><p className="text-sm text-gray-600">Happiness</p></div>
          <div className="bg-white rounded-2xl p-3 text-center shadow-sm"><Brain className="w-5 h-5 text-purple-400 mx-auto mb-1"/><p className="font-bold text-[#1A1A1A]">{wisdom}</p><p className="text-sm text-gray-600">Wisdom</p></div>
        </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-gray-600">Semester Kickoff</p>
          <p className="text-base font-bold text-[#1A1A1A]">Event {eventIdx + 1} of {EVENTS.length}</p>
        </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={eventIdx} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}
          ...
          <span className="text-sm font-semibold text-gray-400 uppercase">Week {ev.week}</span>
          <p className="text-base font-semibold text-[#1A1A1A] mt-1 mb-4">{ev.text}</p>
            ...
            <div className="space-y-2">
              {ev.options.map((opt, i) => (
                <button key={i} onClick={() => advance(opt.cost, opt.happiness, opt.wisdom, opt.text)}
                  ...
                  <span>{opt.text}</span>
                  {opt.cost > 0 && <span className="text-sm text-red-400 ml-2 font-medium">-${opt.cost}</span>}
                </button>
              ))}
            </div>
          )}
