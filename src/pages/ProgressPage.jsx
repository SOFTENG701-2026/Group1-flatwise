    ...
    <div className="space-y-5">
          ...
          <TooltipTrigger asChild>
            <div className="bg-[#1B4332] rounded-2xl p-5 cursor-help">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <p className="text-white/80 text-sm font-semibold">Level {level} — {LEVEL_TITLES[level - 1]}</p>
              ...
              {nextXP && <p className="text-white/70 text-sm mt-1">{nextXP - xp} XP to Level {level + 1}</p>}
            </div>
          </TooltipTrigger>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-white rounded-2xl p-3 text-center shadow-sm">
          ...
          <p className="text-sm text-gray-600">Biases tracked</p>
        </div>
      </div>
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <p className={`text-base font-bold ${unlocked ? 'text-[#1A1A1A]' : 'text-gray-400'}`}>Tier {tier.number}: {tier.title}</p>
                </div>
                <span className={`text-sm font-semibold ${unlocked ? 'text-[#1B4332]' : 'text-gray-400'}`}>{tierDone}/{tierModules.length}</span>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                {tierModules.map(m => (
                  <span key={m.id} className={`text-sm px-2 py-0.5 rounded-full font-medium ${
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          <p className="text-base font-bold text-[#1A1A1A]">Biases to Watch</p>
          <span className="text-sm text-gray-600 bg-gray-100 px-2 py-0.5 rounded-full">Cognitivism</span>
        </div>
        {watchedBiases.length === 0 ? (
          <p className="text-sm text-gray-600 text-center py-3">No biases tracked yet — complete a simulation to detect them.</p>
        ) : (
              ...
              return (
                <div key={biasId} className="flex items-center gap-3 p-2 rounded-xl"
                  ...
                  <div>
                    ...
                    <p className="text-sm" style={{ color: bias.textColor + 'cc' }}>{bias.tagline}</p>
                  </div>
                </div>
              );
            })}
      ...
      {/* Level map */}
      <div className="bg-white rounded-2xl p-4 shadow-sm">
        <p className="text-base font-bold text-[#1A1A1A] mb-3">Level Map</p>
        <div className="grid grid-cols-2 gap-2">
          {LEVEL_TITLES.map((title, i) => (
            <div key={i} className={`p-2.5 rounded-xl text-center ${
              ...
              <p className="text-sm font-bold">Lv {i + 1}</p>
              <p className="text-sm mt-0.5">{title}</p>
              <p className="text-sm opacity-70">{LEVEL_THRESHOLDS[i]} XP</p>
            </div>
          ))}
        </div>
      </div>
